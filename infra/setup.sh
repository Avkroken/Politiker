...
# sätter secrets, deployar app-Workern och installerar bounce-processor.
set -euo pipefail
...
if ! command -v jq >/dev/null; then log "  Installerar jq…"; sudo apt-get update -qq && sudo apt-get install -y jq; fi
if ! command -v python3 >/dev/null; then log "  Installerar python3…"; sudo apt-get install -y python3; fi
ok "Beroenden OK"
...
if [ -z "$(_get MAIL_CRED_KEY)" ]; then _set MAIL_CRED_KEY "$(openssl rand -base64 32)"; ok "Genererade MAIL_CRED_KEY"; fi
...
if ! $WR whoami >/dev/null 2>&1; then warn "Inte inloggad. Öppnar webbläsarinloggning…"; $WR login; fi
...
if ! grep -Fxq "$R2_BUCKET" <<<"$R2_NAMES"; then $WR r2 bucket create "$R2_BUCKET" >/dev/null; fi
ok "R2: $R2_BUCKET"
...
if [ -n "$CUSTOM_DOMAIN" ]; then sed -i "s|$OWNER_DOMAIN|$CUSTOM_DOMAIN|g" "$APP_WR"; ok "Custom domain: $CUSTOM_DOMAIN";
else
  if grep -q '"routes"' "$APP_WR"; then sed -i '/"routes": \[/,/\],/d' "$APP_WR"; fi
...
else ok "  Befintlig databas — hoppar över schema (rör inte din data)"; fi

log "  Kontrollerar databasmigrationer…"
bash "$REPO_DIR/infra/apply-migrations.sh"
ok "  Migrationer kontrollerade"
