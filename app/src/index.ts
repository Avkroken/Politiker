    ...
    if (url.pathname.startsWith("/api/") && resp.status >= 400 && resp.status !== 401 && resp.status !== 404) {
      try {
        ...
        await env.DB.prepare("INSERT INTO worker_errors (id, account_id, method, endpoint, status, error_message, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)")
          .bind(randomId(), account?.id ?? null, req.method, url.pathname, resp.status, errorMessage, Date.now()).run();
      } catch {}
    }
...
const AUTHED_ROUTES: RouteDef[] = [
  ...
  { method: "POST", rx: /^\/api\/draft-letter$/, h: async c => {
    const DAILY_DRAFT_LIMIT=10, rateLimitKey=`draft-rate:${c.accountId}:${new Date().toISOString().slice(0,10)}`, currentCount=parseInt((await c.env.SESSIONS.get(rateLimitKey))??"0",10);
    if(currentCount>=DAILY_DRAFT_LIMIT)return json({error:`Max ${DAILY_DRAFT_LIMIT} AI-utkast per dygn, prova igen imorgon.`},429);
    const {topic,areaType}=await c.req.json<{topic?:string;areaType?:string}>();
    try{const result=await draftLetter(c.env,{topic,areaType});await c.env.SESSIONS.put(rateLimitKey,String(currentCount+1),{expirationTtl:86400});return json(result)}catch(err){return json({error:err instanceof Error?err.message:"Okänt fel"},502)}
  } },
  ...
  { method: "POST", rx: /^\/api\/send$/, h: async c => {
    ...
    const letterId=randomId(); await c.env.DB.prepare("INSERT INTO letters (id, account_id, html_body, created_at) VALUES (?, ?, ?, ?)").bind(letterId,c.accountId,input.letterHtml,Date.now()).run();
    let htmlBody=input.letterHtml; if(input.attachments?.length){const {extractedHtml}=await processAttachments(c.env,letterId,input.attachments);htmlBody+=extractedHtml;await c.env.DB.prepare("UPDATE letters SET html_body = ? WHERE id = ?").bind(htmlBody,letterId).run();}
    return json(await createAndEnqueueSendJob(c.env,c.accountId,{letterId,subject:input.subject,mailCredentialId:input.mailCredentialId,areaNames:input.areaNames,excludeParties:input.excludeParties,excludeEmails:input.excludeEmails,includeRoles:input.includeRoles,includeEmails:input.includeEmails,dailyLimit:input.dailyLimit,switchAfterDays:input.switchAfterDays,nextDailyLimit:input.nextDailyLimit}));
  } },
  ...
  { method: "GET", rx: /^\/api\/public\/letters$/, h: async c => { const page=Math.max(0,parseInt(c.url.searchParams.get("page")??"0",10)); const {results}=await c.env.DB.prepare("SELECT id, source, subject, substr(body, 1, 400) AS excerpt, area_name, published_at FROM public_letters ORDER BY published_at DESC LIMIT 20 OFFSET ?").bind(page*20).all(); return json({letters:results}); } },
  { method: "GET", rx: /^\/api\/public\/letters\/(.+)$/, h: async (c,m) => { const row=await c.env.DB.prepare("SELECT subject, body FROM public_letters WHERE id = ?").bind(m[1]).first<{subject:string;body:string}>(); return row?json(row):json({error:"Hittades inte"},404); } },
  { method: "POST", rx: /^\/api\/letters\/([^/]+)\/publish$/, h: async (c,m) => {
    const letter=await c.env.DB.prepare("SELECT l.id, l.html_body FROM letters l JOIN send_jobs sj ON sj.letter_id = l.id WHERE l.id = ? AND sj.account_id = ? LIMIT 1").bind(m[1],c.accountId).first<{id:string;html_body:string}>(); if(!letter)return json({error:"Brevet hittades inte"},404);
    if(await c.env.DB.prepare("SELECT id FROM public_letters WHERE source = 'user' AND account_id = ?").bind(c.accountId).first())return json({error:"Du har redan publicerat ett brev"},409);
    const subject=(letter.html_body.replace(/[<>]/g,"").split(/\n/).find(l=>l.trim().length>20)??"Medborgarbrev").trim().slice(0,100), pubId=randomId();
    await c.env.DB.prepare("INSERT INTO public_letters (id, source, account_id, subject, body, area_name, published_at) VALUES (?, 'user', ?, ?, ?, NULL, ?)").bind(pubId,c.accountId,subject,letter.html_body.replace(/[<>]/g,""),Date.now()).run(); return json({ok:true,id:pubId});
  } },
];
...
const ADMIN_ROUTES: RouteDef[] = [
  ...
  { method:"POST", rx:/^\/api\/admin\/civic-letter$/, h:async c=>{const{subject,htmlBody,topicSourceUrl}=await c.req.json<{subject:string;htmlBody:string;topicSourceUrl?:string}>();const draft=await createCivicLetterDraft(c.env,{subject,htmlBody,topicSourceUrl});await sendApprovalNotification(c.env,draft);return json({ok:true,draftId:draft.id});} },
  { method:"GET", rx:/^\/api\/admin\/civic-letter\/next-approved$/, h:async c=>{const draft=await getApprovedUnsentDraft(c.env);return json(draft?redactApproveToken(draft):null);} },
  { method:"GET", rx:/^\/api\/admin\/civic-letter\/([a-zA-Z0-9]+)$/, h:async(c,m)=>{const draft=await getCivicLetterDraft(c.env,m[1]);return draft?json(redactApproveToken(draft)):json({error:"Hittades inte"},404);} },
  { method:"POST", rx:/^\/api\/admin\/civic-letter\/([a-zA-Z0-9]+)\/status$/, h:async(c,m)=>{const{status}=await c.req.json<{status:string}>();if(status!=="sending"&&status!=="done")return json({error:"Ogiltig status — måste vara 'sending' eller 'done'"},400);try{await setCivicLetterStatus(c.env,m[1],status)}catch(err){return json({error:err instanceof Error?err.message:"Fel"},400)}return json({ok:true});} },
  ...
  { method:"PATCH", rx:/^\/api\/admin\/public-letters\/([^/]+)$/, h:async(c,m)=>{const{subject,body}=await c.req.json<{subject?:string;body?:string}>();const cleanSubject=(subject??"").trim(),cleanBody=(body??"").trim();if(!cleanSubject||!cleanBody)return json({error:"Ämne och brevtext krävs"},400);const result=await c.env.DB.prepare("UPDATE public_letters SET subject = ?, body = ? WHERE id = ?").bind(cleanSubject,cleanBody,m[1]).run();if(!(result.meta.changes??0))return json({error:"Brevet hittades inte"},404);return json({ok:true});} },
  { method:"DELETE", rx:/^\/api\/admin\/public-letters\/([^/]+)$/, h:async(c,m)=>{const result=await c.env.DB.prepare("DELETE FROM public_letters WHERE id = ?").bind(m[1]).run();if(!(result.meta.changes??0))return json({error:"Brevet hittades inte"},404);return json({ok:true});} },
...
async function handleRequest(req: Request, env: Env, url: URL): Promise<Response> {
  ...
  if(civicLetterMatch){const[,draftId,action]=civicLetterMatch,token=url.searchParams.get("token");if(!token)return json({error:"Saknar token"},400);try{if(action==="approve"){await approveCivicLetterDraft(env,draftId,token);return new Response("Godkänt — brevet skickas i kommande dagliga omgångar.",{status:200,headers:{"Content-Type":"text/plain; charset=utf-8"}})}await rejectCivicLetterDraft(env,draftId,token);return new Response("Avslaget — inget skickas.",{status:200,headers:{"Content-Type":"text/plain; charset=utf-8"}})}catch(err){return json({error:err instanceof Error?err.message:"Fel"},400)}}
  ...
  try{
    ...
    if(url.pathname==="/api/newsletter/subscribe"&&req.method==="POST"){const{email,turnstileToken}=await req.json<{email:string;turnstileToken?:string}>();if(!(await verifyTurnstile(env.TURNSTILE_SECRET,turnstileToken,req.headers.get("CF-Connecting-IP"))))return json({error:"Bekräfta att du inte är en robot och försök igen."},400);await subscribeNewsletter(env,email);return json({ok:true});}
    if(url.pathname==="/api/newsletter/confirm"&&req.method==="GET")return confirmNewsletter(env,url.searchParams.get("id"),url.searchParams.get("token"));
    if(url.pathname==="/api/newsletter/unsubscribe"&&req.method==="GET")return unsubscribeNewsletter(env,url.searchParams.get("id"),url.searchParams.get("token"));
