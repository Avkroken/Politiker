...
export async function exportAdminData(env:Env,section:"accounts"|"feedback"|"stats"|"public_contacts"|"politicians"|"all",format:"csv"|"json"):Promise<{filename:string;content:string;contentType:string}>{
  const accountsRows=async()=>{const{results}=await env.DB.prepare("SELECT id, email, is_admin, email_verified, disabled, daily_send_cap, created_at FROM accounts ORDER BY created_at").all<Record<string,unknown>>();return results};
  const feedbackRows=async()=>{const{results}=await env.DB.prepare("SELECT id, account_id, message, github_issue_url, created_at FROM feedback ORDER BY created_at DESC").all<Record<string,unknown>>();return results};
  const publicContactRows=async()=>{const{results}=await env.DB.prepare("SELECT id, name, email, area_name, area_type, party, role, organisation, unit, title, academic_field, source_url, verification_status, last_scraped_at, last_verified_at FROM public_contacts ORDER BY area_type, area_name, name").all<Record<string,unknown>>();return results};
  let data:Record<string,unknown>|Record<string,unknown>[],baseName:string;
  if(section==="accounts"){data=await accountsRows();baseName="konton"}
  else if(section==="feedback"){data=await feedbackRows();baseName="feedback"}
  else if(section==="public_contacts"||section==="politicians"){data=await publicContactRows();baseName="offentliga-mottagare"}
  else if(section==="stats"){const stats=await getAdminStats(env);data=format==="csv"?stats.dailySeries:stats as unknown as Record<string,unknown>;baseName="statistik"}
  else{const[accounts,feedback,stats,publicContacts]=await Promise.all([accountsRows(),feedbackRows(),getAdminStats(env),publicContactRows()]);data={accounts,feedback,stats,public_contacts:publicContacts};baseName="allt"}
  const date=new Date().toISOString().slice(0,10);
  if(format==="json")return{filename:`politiker-${baseName}-${date}.json`,content:JSON.stringify(data,null,2),contentType:"application/json"};
  ...
  return{filename:`politiker-${baseName}-${date}.csv`,content:toCsv(rows),contentType:"text/csv"};
}
