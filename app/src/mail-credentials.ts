...
export const PROVIDER_PRESETS: Record<string, { host: string; port: number; helpUrl: string; providerDailyLimit: number | null }> = {
  gmail: { host: "smtp.gmail.com", port: 587, helpUrl: "https://myaccount.google.com/apppasswords", providerDailyLimit: 100 },
  outlook: { host: "smtp.office365.com", port: 587, helpUrl: "https://account.live.com/proofs/AppPassword", providerDailyLimit: 300 },
  icloud: { host: "smtp.mail.me.com", port: 587, helpUrl: "https://appleid.apple.com/account/manage", providerDailyLimit: 1000 },
  yahoo: { host: "smtp.mail.yahoo.com", port: 587, helpUrl: "https://login.yahoo.com/account/security", providerDailyLimit: 500 },
...
export async function addMailCredential(
  ...
  let host: string;
  let port: number;
  if (input.provider === "generic") ({ host, port } = validateGenericSmtpEndpoint(input.host, input.port));
  else { host = preset.host; port = preset.port; }

  const user = assertNoControlCharacters(input.user ?? "", "SMTP-användarnamn");
  if (user.length > 320) throw new Error("Ogiltigt SMTP-användarnamn");
  const fromAddress = validateMailboxAddress(input.fromAddress ?? "", "avsändaradress");
  ...
  await env.DB.prepare(
    `INSERT INTO mail_credentials (id, account_id, provider, smtp_host, smtp_port, smtp_user, encrypted_password, from_address, verified_at, daily_cap, user_cap_pct, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).bind(id, accountId, input.provider, host, port, user, encryptedPassword, fromAddress, Date.now(), dailyCap, userCapPct, Date.now()).run();
  return { id, dailyCap };
}
...
export async function addMicrosoftGraphMailCredential(env: Env, accountId: string, code: string): Promise<{ id: string }> {
  ...
  const email = validateMailboxAddress(tokens.email, "Microsoft-kontots e-postadress");
  const id = randomId();
  ...
  ).bind(id, accountId, email, email, Date.now(), dailyCap, encryptedAccessToken, encryptedRefreshToken, tokens.expiresAt, Date.now()).run();
  return { id };
}
...
export async function updateMailCredentialCapPct(env: Env, accountId: string, credentialId: string, userCapPct: number): Promise<{ dailyCap: number | null }> {
  const cred = await env.DB.prepare("SELECT provider FROM mail_credentials WHERE id = ? AND account_id = ? AND revoked_at IS NULL")
    .bind(credentialId, accountId).first<{ provider: string }>();
  if (!cred) throw new Error("Mailkonto saknas eller är borttaget");
  const dailyCap = computeDailyCap(cred.provider, userCapPct);
  await env.DB.prepare("UPDATE mail_credentials SET user_cap_pct = ?, daily_cap = ? WHERE id = ? AND account_id = ? AND revoked_at IS NULL")
    .bind(Math.min(100, Math.max(1, Math.round(userCapPct))), dailyCap, credentialId, accountId).run();
  return { dailyCap };
}
...
export async function deleteMailCredential(env: Env, accountId: string, credentialId: string): Promise<void> {
  const credential = await env.DB.prepare("SELECT id FROM mail_credentials WHERE id = ? AND account_id = ? AND revoked_at IS NULL")
    .bind(credentialId, accountId).first<{ id: string }>();
  if (!credential) return;
  const now = Date.now();
  await env.DB.batch([
    env.DB.prepare(`UPDATE send_jobs SET status = 'cancelled', finished_at = ? WHERE account_id = ? AND mail_credential_id = ? AND status IN ('pending', 'sending')`).bind(now, accountId, credentialId),
    env.DB.prepare(`UPDATE send_job_recipients SET status = 'cancelled', queued_at = NULL, finished_at = ? WHERE send_job_id IN (SELECT id FROM send_jobs WHERE account_id = ? AND mail_credential_id = ?) AND status IN ('pending', 'queued')`).bind(now, accountId, credentialId),
    env.DB.prepare(`UPDATE mail_credentials SET encrypted_password = '', oauth_access_token = NULL, oauth_refresh_token = NULL, oauth_token_expires_at = NULL, revoked_at = ? WHERE id = ? AND account_id = ? AND revoked_at IS NULL`).bind(now, credentialId, accountId),
  ]);
}
