...
async function takeRateLimit(env: Env, key: string, capacity: number, refillPerMinute: number): Promise<boolean> {
  ...
  try {
    const response = await env.RATE_LIMITER.get(id).fetch("https://rate-limiter/acquire", {
      method: "POST",
      body: JSON.stringify({ capacity, refillPerMinute }),
    });
    const result = await response.json<{ granted?: boolean }>();
    return result.granted === true;
  } catch {
    // Missbruksgränser ska inte kunna kringgås genom att limitern fallerar.
    return false;
  }
}
...
async function allowPublicWrite(req: Request, env: Env, pathname: string): Promise<boolean> {
  ...
  if (pathname === "/api/feedback") {
    return (await takeRateLimit(env, `feedback:${ip}`, 10, 2))
      && (await takeRateLimit(env, "feedback:global", 120, 60));
  }
  return (await takeRateLimit(env, `client-error:${ip}`, 30, 10))
    && (await takeRateLimit(env, "client-error:global", 300, 120));
}
...
async function upgradeOAuthSession(req: Request, env: Env, response: Response): Promise<void> {
  ...
  try {
    await writeSession(env, token, raw, Date.now());
  } catch (error) {
    await env.SESSIONS.delete(`session:${token}`).catch(() => {});
    throw error;
  }
}
...
async function secureFetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  // API-nycklar är programmatisk sändningsbehörighet, inte permanenta
  // webbsessioner. En giltig sessionskaka får vinna eftersom basroutern också
  // prioriterar den framför Authorization-headern.
  if (bearer && url.pathname.startsWith("/api/") && !apiKeyRouteAllowed(req.method, url.pathname)) {
    const sessionToken = getCookie(req, "session");
    const session = sessionToken ? await getSessionContext(env, sessionToken) : null;
  ...
  if (needsFreshSession(req.method, url.pathname)) {
    ...
    if (Date.now() - session.authenticatedAt > FRESH_AUTH_MS) {
      return withSecurityHeaders(json({ error: "Logga ut och in igen innan du ändrar kontots säkerhetsinställningar" }, 403));
    }
  }
  ...
  if (req.method === "POST" && (url.pathname === "/api/feedback" || url.pathname === "/api/client-error")) {
    ...
    if (Number.isFinite(contentLength) && contentLength > maxBytes) {
      return withSecurityHeaders(json({ error: "För stor begäran" }, 413));
    }
    if (!(await allowPublicWrite(req, env, url.pathname))) {
      return withSecurityHeaders(json({ error: "För många anrop — försök igen senare" }, 429));
    }
  }

  const response = await baseApp.fetch(req, env, ctx);
  try {
    await upgradeOAuthSession(req, env, response);
  } catch {
    return withSecurityHeaders(json({ error: "Kontot kunde inte skapa en giltig session" }, 403));
  }
