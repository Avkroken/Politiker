...
async function isRelevant(item: MonitoredItem, apiKey: string, db: D1Database): Promise<boolean> {
  const answer = await callAnthropic(apiKey, { model: ANTHROPIC_HAIKU, maxTokens: 5, prompt: `Avgör om följande nyhet eller riksdagsärende har tydlig betydelse för allmänintresset och svenska invånares vardag, offentliga resurser eller långsiktiga samhällsfunktioner.
...
Titel: ${item.title}
Sammanfattning: ${(item.summary ?? "").slice(0, 500)}`, budget: LETTER_GEN_CALL_BUDGET }, db);
  return answer.toLowerCase().startsWith("ja");
}

async function generateLetter(item: MonitoredItem, pol: Politician, apiKey: string, db: D1Database): Promise<string> {
  ...
  return callAnthropic(apiKey, { model: ANTHROPIC_SONNET, maxTokens: 950, prompt: `Du skriver för ${EDITORIAL_NAME}, en oberoende redaktionell funktion på Politikerkontakt.
...
async function relevantRecipients(env: Env, item: MonitoredItem): Promise<Politician[]> {
  if (item.area_type === "region") {
    ...
    const { results } = await env.DB.prepare(
      `SELECT id,name,email,area_name,party,role FROM politicians
       WHERE area_type='region' AND (area_name LIKE ? OR area_name LIKE ?)
         AND (verification_status IS NULL OR verification_status!='dead_via_send')
       ORDER BY CASE WHEN role LIKE '%ordför%' THEN 0 ELSE 1 END, RANDOM() LIMIT ?`,
    ).bind(`%${kw}%`, `%${item.area_name ?? ""}%`, MAX_RECIPIENTS_PER_ITEM).all<Politician>();
    return results;
  }
  const { results } = await env.DB.prepare(
    `SELECT id,name,email,area_name,party,role FROM politicians
     WHERE area_type IN ('riksdag','regering')
       AND (verification_status IS NULL OR verification_status!='dead_via_send')
     ORDER BY CASE WHEN area_type='regering' THEN 0 ELSE 1 END,
              CASE WHEN role LIKE '%minister%' OR role LIKE '%ordför%' THEN 0 ELSE 1 END,
              RANDOM() LIMIT ?`,
  ).bind(MAX_RECIPIENTS_PER_ITEM).all<Politician>();
  return results;
}
...
export async function runLetterGenerator(env: Env): Promise<void> {
  ...
  for (const item of items) {
    ...
    try { relevant = await isRelevant(item, env.ANTHROPIC_API_KEY, env.DB); }
    catch (e) {
    ...
    const politicians = await relevantRecipients(env, item);
    if (!politicians.length) { await env.DB.prepare("UPDATE monitored_items SET letter_queued=2 WHERE id=?").bind(item.id).run(); continue; }

    let itemDrafts = 0;
    const subject = makeSubject(item);
    for (const pol of politicians) {
      try {
        const body = await generateLetter(item, pol, env.ANTHROPIC_API_KEY, env.DB);
        const encryptedBody = await encryptLetterData(env, body);
        const draftId = randomId();
        const approveToken = randomId() + randomId();
        const now = Date.now();
        await env.DB.batch([
          env.DB.prepare("INSERT INTO civic_letter_drafts (id,subject,html_body,topic_source_url,status,approve_token,created_at) VALUES (?,?,?,?,'pending',?,?)")
            .bind(draftId, subject, encryptedBody, item.url, approveToken, now),
          env.DB.prepare("INSERT INTO campaign_recipients (id,draft_id,politician_id,politician_email,politician_name,area_name) VALUES (?,?,?,?,?,?)")
            .bind(randomId(), draftId, pol.id, pol.email, pol.name, pol.area_name),
        ]);
        await sendApprovalNotification(env, { id: draftId, subject, htmlBody: body, topicSourceUrl: item.url, status: "pending", approveToken, createdAt: now, approvedAt: null });
        totalDrafts++; itemDrafts++;
      } catch (e) {
        if (e instanceof AnthropicBudgetExceededError) {
