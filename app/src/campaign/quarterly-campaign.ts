...
export async function runQuarterlyCampaign(env: Env): Promise<void> {
  ...
  const existing = await env.DB.prepare(
    `SELECT id FROM civic_letter_drafts WHERE topic_source_url=? AND created_at>=? LIMIT 1`,
  ).bind(QUARTERLY_MARKER, quarterStart).first();
  if (existing) { console.log("quarterly: kvartalets utkast finns redan"); return; }

  const { results: items } = await env.DB.prepare(
    "SELECT title,summary,url,source FROM monitored_items WHERE letter_queued=1 AND created_at>=? ORDER BY CASE WHEN source='riksdagen' THEN 0 WHEN source LIKE 'svt_%' THEN 1 ELSE 2 END, created_at DESC LIMIT ?",
  ...
  try {
    raw = await callAnthropic(env.ANTHROPIC_API_KEY, {
      ...
      maxTokens: 2200,
      prompt: `Du skriver för ${EDITORIAL_NAME}, en oberoende redaktionell funktion på Politikerkontakt. Brevet går till förtroendevalda på flera nivåer i Sverige.
...
<brevtexten>`,
  ...
  ).bind(draftId, subject, await encryptLetterData(env, body), QUARTERLY_MARKER, approveToken, now).run();

  const res = await env.DB.prepare(`
    INSERT INTO campaign_recipients (id,draft_id,politician_id,politician_email,politician_name,area_name)
    SELECT lower(hex(randomblob(16))), ?, id, email, name, area_name FROM politicians
    WHERE verification_status IS NULL OR verification_status NOT IN ('dead','dead_via_send') GROUP BY email
  `).bind(draftId).run();
...
export async function runQuarterlyDrain(env: Env): Promise<void> {
  ...
  const { results } = await env.DB.prepare(`SELECT cr.id,cr.politician_email,cld.subject,cld.html_body FROM campaign_recipients cr JOIN civic_letter_drafts cld ON cld.id=cr.draft_id WHERE cr.status='pending' AND cld.status='approved' AND cld.topic_source_url=? ORDER BY cr.rowid ASC LIMIT ?`).bind(QUARTERLY_MARKER,Math.min(DRAIN_PER_RUN,remainingMonthly)).all<{id:string;politician_email:string;subject:string;html_body:string}>();
  let sent=0,failed=0;
