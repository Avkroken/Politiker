...
function loadExplicitEmailJsonChunks(): (values: string[]) => string[] {
  const start = dbSource.indexOf('function explicitEmailJsonChunks');
  const endMarker = '\n}\n\nexport async function getRecipientsForAreas';
  const end = dbSource.indexOf(endMarker, start);
  assert.ok(start >= 0 && end > start, 'chunk helper must exist before getRecipientsForAreas');
  ...
  runInNewContext(`${functionSource}; chunker=explicitEmailJsonChunks;`, context);
